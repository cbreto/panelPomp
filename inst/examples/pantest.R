...
pomp::pomp(data=data.frame(t=1:20,Y=NA),
           ...
           rmeasure=rmeas,
           dmeasure=dmeas,
           params=c(r=0.1,sigma=0.1,X.0=1),
...
U <- 2
#simulate data above
#simTests <- setNames(vector(mode="list",length=U),nm=paste0("unit",1:U))
#for (i.u in seq_len(U)) simTests[[i.u]] <- pomp::simulate(test,seed=0+i.u)
#simTest <- as.data.frame(sapply(simTests,obs))
#write.table(simTest,"simTest.txt",sep="\t",row.names=FALSE)
poList <- setNames(vector(mode="list",length=U),
                   nm=paste0("unit",1:U))
for (i.u in seq_len(U)) {
  test_u <- pomp::pomp(
    data.frame(t=time(test),Y=as.numeric(simTest[,i.u])),
    times="t",
    t0=timezero(test)
  )
  poList[[i.u]] <- test
  poList[[i.u]]@data <- test_u@data
}
## Construct panelPomp
panelPomp(poList,
          shared=coef(test)[c("r","sigma") -> shnm],
          specific=matrix(coef(test)[!names(coef(test))%in%shnm] -> spparams,
